...
API_TIMEOUT = 10  # seconds (기존 5초에서 증가)
...
def print_progress_stats():
    ...
    print("\n" + "-"*60)
    print(f"⏱️  Runtime: {elapsed_hours:.2f}h | Processed: {stats['total_processed']} videos")
    print(f"📊 Memory: {psutil.virtual_memory().percent:.1f}% | Errors: {stats['total_errors']}")
    print("-"*60 + "\n")

# --- [메인 실행] ---
if __name__ == "__main__":
    print("="*80)
    print("🚀 LONG-RUN VIDEO ANALYSIS SYSTEM")
    print("="*80)
                ...
                if curr_t - last_capture_time > 0.25:
                    ...
                    if len(frame_buffer) == WINDOW_SIZE and new_frame_count >= STRIDE:
                        if not is_processing:
                            ...
                            threading.Thread(target=request_inference, 
                                           args=(list(frame_buffer), current_frame_pos, fps), 
                                           daemon=True).start()
